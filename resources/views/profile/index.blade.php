...
══════════════════════════════════════════ -->
    <div class="tabs-bar">
        <div class="tabs-inner">
            ...
            @if(isset($bookmarkedProjects) && $bookmarkedProjects->count() > 0)
            <div class="tab" data-tab="bookmarks">
                Proyectos Favoritos
                <span class="tab-count">{{ $bookmarkedProjects->count() }}</span>
            </div>
            @endif
        </div>
    </div>
...
══════════════════════════════════════════ -->
    ...
    <div class="content" data-panel="progress">
        ...
        $isProjectOwner = auth()->check() && $project->user_id === auth()->id();
        @endphp
        <div class="p-card" data-project-id="{{ $project->id }}">
            ...
            <div class="card-head">
                ...
                <div class="drop-wrap">
                    <button class="card-menu post-menu-btn" aria-label="Opciones del proyecto" data-project-id="{{ $project->id }}">
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    {{-- Panel: Favoritos --}}
    @if(isset($bookmarkedProjects) && $bookmarkedProjects->count() > 0)
    <div class="content" data-panel="bookmarks" style="display:none">
        @forelse ($bookmarkedProjects ?? collect() as $project)
        @php
        $isLiked = $project->precomputed_is_liked ?? false;
        $skillCounts = $project->skillEndorsements->groupBy('skill_type')->map->count()->toArray();
        $userEndorsement = $project->precomputed_user_endorsement ?? null;
        ...
        <div class="p-card" data-project-id="{{ $project->id }}">
            ...
            <div class="update-block">
                <img src="{{ $project->user->avatar_url }}"
        ...
        @empty
        <p class="empty-state">No hay proyectos en favoritos.</p>
        @endforelse
    </div>
    @endif
    <!-- end content -->
